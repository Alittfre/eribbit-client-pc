    ...
    <div class="container">
      ...
      <div class="cart">
        ...
        <table>
          <thead>
            <tr>
              <th width="120"><XtxCheckbox :modelValue="isCheckAll">全选</XtxCheckbox></th>
          ...
          <tbody>
            <tr v-for="item in $store.getters['cart/validList']" :key="item.skuId">
              <td><XtxCheckbox :modelValue="item.selected" @change="$store.dispatch('cart/updateCart', item)"/></td>
              <td>
                <div class="goods">
              ...
              <td class="tc">
                <XtxNumbox :modelValue="item.count" @change="$store.dispatch('cart/updateCart', item)" />
              </td>
      ...
      <div class="action">
        <div class="batch">
          <XtxCheckbox :modelValue="isCheckAll">全选</XtxCheckbox>
...
export default {
  ...
  setup () {
    ...
    const isCheckAll = computed({
      ...
      set: (selected) => store.dispatch('cart/checkAllCart', selected)
    })
    // 删除
    const deleteCart = (skuId) => {
      const bl = confirm('是否删除？')
      if (bl) store.dispatch('cart/deleteCart', skuId)
    }
    // 批量删除
    ...
    return { isCheckAll, deleteCart, batchDeleteCart }
  }
}
</script>
